...
id: portabase
name: Portabase
description: 用于简化数据库实例备份和恢复的工具，支持 PostgreSQL、MySQL、MariaDB、MongoDB、SQLite、Redis 和 Valkey。
tags:
  - 数据库
  - 备份
  - Docker
...
Portabase 是一款开源的自托管数据库备份与恢复平台，旨在简化数据库实例的备份管理。它通过部署在数据库容器旁边的轻量级 [Portabase Agent](https://github.com/Portabase/agent-rust) 安全、高效地处理备份操作，所有任务均可通过简洁的 Web 控制台统一管理。
